...
def _approval_event() -> InternalApprovalEvent:
    row = json.loads(
        (RENEWAL_ROOT / "internal_approval_events.jsonl").read_text().splitlines()[0]
    )
    row["approved_scope"] = {
        "action": "expand",
        "new_seat_limit": 50,
        "annual_cost_ceiling_usd": 22000,
        "term_start_date": "2026-07-21",
        "term_end_date": "2027-07-20",
    }
...
def _reply(fixture_id: str) -> str:
    return (RENEWAL_ROOT / "provider_replies" / f"{fixture_id}.eml").read_text(
        encoding="utf-8"
    )
...
def _restart(model: FakeProviderModel):
    ...
    return build_renewal_workflow(
        extractor=ProviderDecisionExtractor(model=model), security_reviewer_id="E006"
    )
...
def test_approved_renewal_survives_restarts_and_replays_exactly_once() -> None:
    ...
    started = build_renewal_workflow(
        extractor=ProviderDecisionExtractor(model=model), security_reviewer_id="E006"
    ).start("2026-07-01")
    ...
    assert approved.stage == "awaiting_internal_approval"
    for line in (
        (ROOT / "evals/fixtures/lesson15/internal_approval_events.jsonl")
        .read_text()
        .splitlines()[1:]
    ):
        _restart(model).handle_internal_event(InternalApprovalEvent(**json.loads(line)))

    workflow = _restart(model)
    result = workflow.handle_provider_reply(
        started.run_id, "webex_renewal_approved", _reply("webex_renewal_approved")
    )
    replay = workflow.handle_provider_reply(
        started.run_id, "webex_renewal_approved", _reply("webex_renewal_approved")
    )

    assert result.stage == "awaiting_effective_date"
    assert result.applied is False
    result = workflow.start("2026-07-21")
    replay = workflow.start("2026-07-21")
...
def test_conditional_and_ambiguous_replies_make_no_business_update() -> None:
    for fixture_id in ("webex_renewal_conditional", "webex_renewal_ambiguous"):
        ops.reset_conn()
        model = FakeProviderModel()
        workflow = build_renewal_workflow(
        ...
        result = workflow.handle_provider_reply(
            started.run_id, fixture_id, _reply(fixture_id)
        )
...
def test_wrong_approver_and_mismatched_contract_fail_closed() -> None:
    ...
    mismatched = build_renewal_workflow(
        extractor=ProviderDecisionExtractor(model=MismatchModel())
    )
    mismatched_source = _reply("webex_renewal_approved").replace("C001", "C999")
    result = mismatched.handle_provider_reply(
        started.run_id, "webex_renewal_approved", mismatched_source
    )
    assert result.status == "needs_human"
    assert result.applied is False
