...
def main() -> int:
    ...
    schedule.add_argument(
        "--as-of", default=os.getenv("NOVAOPS_AS_OF", date.today().isoformat())
    )
    ...
    if args.command == "approval-event":
        event = InternalApprovalEvent(
            **json.loads(args.event_file.read_text(encoding="utf-8"))
        )
    ...
    if args.command == "provider-reply":
        ...
        _print(
            build_renewal_workflow_from_env(
                replies={args.fixture_id: reply}
            ).handle_provider_reply(args.run_id, args.fixture_id, reply)
        )
        return 0

    while True:
        as_of = os.getenv("NOVAOPS_AS_OF", date.today().isoformat())
